...
<form action="<?php echo url_for('aluno/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          &nbsp;<?php echo link_to(__('Voltar para a lista'), url_for('aluno/index')); ?>
          <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to(__('Excluir'), 'aluno/delete?id='.$form->getObject()->getId(), array('method' => 'delete', 'confirm' => _('Você tem certeza que deseja excluir?'))) ?>
          <?php endif; ?>
